...
const changeHelloButton = ()=> {
    ...
    //how to access style on the say hello button
    document.getElementById('#say-hello-button')
...
sayHelloButton.addEventListener('mouseout',changeHelloButtonBack)
...
const ohMy = () => {
    ...
    .then((res) => {
        ...
        for(let i = 0; i < res.data.length; i++){
            const node = document.createElement("p") // creates new paragraph element
            node.textContent = (res.data[i]) // sets the text of paragraph to current item
            document.body.appendChild(node); // append the paragraph node to the body
...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/${'yomomma'}`)
    .then((res) => {
...
// CODE HERE
const queryTest = () => {
    axios.get(`http://localhost:3000/query-test/?fruits=banana&yo=mama`)
    .then((res) => {
        console.log(res.data)
    })  
}
...
// it console.logs the following message You sent an empty query!

// 2: Send more than 1 query on the URL -- what happened? 

// it console logs the following message:{message: "You sent more than 1 query!", queries: {fruits: "banana", yo: "mama"}}
...
/*
    ...
    const createFood = () => {
        const foodInput = document.getBy
    }
